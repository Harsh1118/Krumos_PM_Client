...
export const Projects: React.FC = () => {
  ...
  const { data: projects = [], isLoading: loading } = useQuery<Project[]>({
    queryKey: ['projects', activeWorkspace?.slug],
    queryFn: async () => {
      const res = await api.get(`/workspaces/${activeWorkspace?.slug}/projects`);
      return res.data;
    },
    enabled: !!activeWorkspace?.slug,
  });

  // Project Creation/Edit Mutation
  const submitProjectMutation = useMutation({
    mutationFn: async ({ mode, name, description, projectId }: { mode: 'create' | 'edit'; name: string; description: string; projectId?: string }) => {
      if (mode === 'create') {
        const res = await api.post(`/workspaces/${activeWorkspace?.slug}/projects`, {
          name,
          description,
        });
        return res.data;
        ...
        const res = await api.patch(
          `/workspaces/${activeWorkspace?.slug}/projects/${projectId}`,
          { name, description }
        );
        return res.data;
      }
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['projects', activeWorkspace?.slug] });
      toast.success(variables.mode === 'create' ? `Project "${data.name}" created successfully` : `Project "${data.name}" updated successfully`);
  ...
  const submitting = submitProjectMutation.isPending;
  ...
  // Archive/Restore Mutation (Optimistic Update)
  const archiveToggleMutation = useMutation({
    mutationFn: async (project: Project) => {
      const res = await api.patch(`/workspaces/${activeWorkspace?.slug}/projects/${project.id}`, {
        isArchived: !project.isArchived,
      });
      return res.data;
    },
    onMutate: async (project) => {
      await queryClient.cancelQueries({ queryKey: ['projects', activeWorkspace?.slug] });
      const previousProjects = queryClient.getQueryData<Project[]>(['projects', activeWorkspace?.slug]);

      if (previousProjects) {
        queryClient.setQueryData<Project[]>(
          ['projects', activeWorkspace?.slug],
          previousProjects.map((p) => (p.id === project.id ? { ...p, isArchived: !p.isArchived } : p))
        );
      }
    ...
    onError: (_err, _variables, context) => {
      if (context?.previousProjects) {
        queryClient.setQueryData(['projects', activeWorkspace?.slug], context.previousProjects);
      }
  ...
  const handleArchiveToggle = async (project: Project) => {
    archiveToggleMutation.mutate(project);
  };

  // Delete Mutation (Optimistic Update)
  const deleteProjectMutation = useMutation({
    mutationFn: async (projectId: string) => {
      await api.delete(`/workspaces/${activeWorkspace?.slug}/projects/${projectId}`);
    },
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    ...
                    {isManagerOrAdmin && (
                      <>
                        ...
                        <button
                          ...
                          disabled={archiveToggleMutation.isPending && archiveToggleMutation.variables?.id === p.id}
                        >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        disabled={deleteProjectMutation.isPending && deleteProjectMutation.variables === p.id}
                      >
                        <Trash2 size={14} />
                      </button>
                    )}
