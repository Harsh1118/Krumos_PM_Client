...
import { useDashboardQuery, useDashboardAnalyticsQuery } from "../../api/dashboard/useDashboardApi";
...
export const Dashboard: React.FC = () => {
  ...
  // 1. Fetch dashboard data using custom hook
  const { data: dashboardData, isLoading: dashboardLoading } = useDashboardQuery(activeWorkspace?.slug);

  // 2. Fetch analytics data using custom hook (Manager/Admin only)
  const { data: analyticsData, isLoading: analyticsLoading } = useDashboardAnalyticsQuery(
                    ...
                    {myTasks.map((t: UserTask) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <ArrowRight
                            className="text-mute opacity-0 -translate-x-1 transition-all duration-155 ml-2 group-hover:opacity-100 group-hover:translate-x-0 shrink-0 hidden sm:block"
                            size={14}
                          />
                        </div>
                      );
